...
def calc_metrics(raw: dict) -> dict:  # noqa: C901
    ...
    m["div_rate"]        = _i(info, "dividendRate")   # abs. Dividende je Aktie p.a.
    m["country"]         = _i(info, "country") or ""
    m["website"]         = _i(info, "website")

    # Dividend history metrics
    div_hist = raw.get("div_hist", pd.Series(dtype=float))
    m["div_hist_raw"] = div_hist
    if not div_hist.empty:
        try:
            # Jährliche Summen (Zeitzone entfernen für Kompatibilität)
            _dh = div_hist.copy()
...
def calc_jack(m: dict) -> dict:
    ...
    # --- DIVIDENDEN-CHECK ---
    j["dividenden_check"] = _calc_dividenden_check(m)
...
# ── DIVIDENDEN-CHECK ──────────────────────────────────────────────────────────
def _calc_dividenden_check(m: dict) -> dict:
    """JACK 💰 Dividenden-Check: Nachhaltigkeit · Flags · Malus · Verdikt.
    AUTO-TRIGGER: nur aktiv wenn Dividende gezahlt wird (div_yield > 0).
    """
    div_yield = m.get("dividend") or 0
    if div_yield <= 0:
        return {"active": False}

    # Quellensteuertabelle (Standard: 15%)
    WITHHOLDING = {
        "United States": 0.15, "Germany": 0.2638, "Switzerland": 0.35,
        "United Kingdom": 0.00, "Netherlands": 0.15, "France": 0.128,
        "Canada": 0.25, "Australia": 0.30, "Japan": 0.15,
        "Sweden": 0.30, "Denmark": 0.27, "Norway": 0.25,
        "Finland": 0.30, "Belgium": 0.30, "Spain": 0.19,
        "Italy": 0.26, "Ireland": 0.25, "Luxembourg": 0.15,
        "Singapore": 0.00, "Hong Kong": 0.00, "China": 0.10,
        "South Korea": 0.20, "Taiwan": 0.21, "Brazil": 0.15,
        "Mexico": 0.10,
    }
    country     = m.get("country") or ""
    wht         = WITHHOLDING.get(country, 0.15)
    netto_yield = div_yield * (1.0 - wht)

    # Absolute Ausschüttung
    div_rate    = m.get("div_rate") or 0
    shares_out  = m.get("shares_out") or 0
    div_payments = div_rate * shares_out   # jährliche Gesamt-Ausschüttung
    ...
    # Payout-Quoten
    payout_eps  = m.get("payout_ratio")
    fcf_payout  = (div_payments / fcf) if (fcf > 0 and div_payments > 0) else None
    fcf_coverage = (real_fcf / div_payments) if (div_payments > 0 and real_fcf > 0) else None

    # Historik
    div_years   = m.get("div_years") or 0
    div_cagr5   = m.get("div_cagr5")
    div_cuts    = m.get("div_cuts") or 0

    # Weitere Kennzahlen
    nd_ebitda    = m.get("nd_ebitda")
    capex_ratio  = m.get("capex_ratio") or 0
    interest_exp = abs(m.get("interest_expense") or 0)
    rev_growth   = m.get("revenue_growth") or 0
    sector       = (m.get("sector") or "").lower()
    buyback      = m.get("buyback_yield_est") or 0
    sbc_i        = m.get("sbc_intensity") or 0

    # ── Spezial-Badges ────────────────────────────────────────────────────────
    is_aristocrat = (div_years >= 25 and div_cuts == 0)
    is_neustart   = (0 < div_years <= 3)

    # ── Flags (name, malus, color, detail) ────────────────────────────────────
    flags = []
    malus = 0.0

    # 1 – DIV-TRAP: Rendite > 8% bei unzureichender Coverage
    if div_yield > 0.08 and (fcf_coverage is None or fcf_coverage < 2.0):
        _cov_txt = (f"FCF-Coverage {fcf_coverage:.1f}x" if fcf_coverage is not None
                    else "FCF-Coverage N/V")
        flags.append(("DIV-TRAP", -2, "#da3633",
                      f"Rendite {div_yield:.1%} > 8% bei {_cov_txt}"))
        malus -= 2

    # 2 – CASH-DRAIN: Div-Zahlungen übersteigen Real FCF
    if div_payments > 0 and real_fcf < div_payments:
        _rf_txt = f"{real_fcf/1e9:.2f}B" if abs(real_fcf) >= 1e8 else f"{real_fcf/1e6:.0f}M"
        _dp_txt = f"{div_payments/1e9:.2f}B" if div_payments >= 1e8 else f"{div_payments/1e6:.0f}M"
        flags.append(("CASH-DRAIN", -2, "#da3633",
                      f"Div-Zahlungen ({_dp_txt}) > Real FCF ({_rf_txt}) — Substanzabbau"))
        malus -= 2

    # 3 – DIV-STRESS: FCF-Coverage < 1.5x (nur wenn kein CASH-DRAIN)
    has_drain = any(f[0] == "CASH-DRAIN" for f in flags)
    if fcf_coverage is not None and fcf_coverage < 1.5 and not has_drain:
        flags.append(("DIV-STRESS", -1, "#d29922",
                      f"FCF-Coverage {fcf_coverage:.1f}x < 1.5x — Puffer zu gering"))
        malus -= 1

    # 4 – DEBT-FINANZIERT: ND/EBITDA > 3x bei laufender Dividende
    if nd_ebitda is not None and nd_ebitda > 3.0 and div_payments > 0:
        flags.append(("DEBT-FINANZIERT", -1, "#d29922",
                      f"Net Debt/EBITDA {nd_ebitda:.1f}x > 3.0x bei laufender Dividende"))
        malus -= 1

    # 5 – CAPEX-KONFLIKT: hohes Capex + hohe Ausschüttungsquote
    if capex_ratio > 0.20 and fcf_payout is not None and fcf_payout > 0.80:
        flags.append(("CAPEX-KONFLIKT", -1, "#d29922",
                      f"Capex/Umsatz {capex_ratio:.1%} · FCF-Payout {fcf_payout:.0%}"))
        malus -= 1

    # 6 – ÜBERDEHNUNG: Div-CAGR > 15% (überhitzt) oder deutlich über FCF-CAGR
    if div_cagr5 is not None and div_cagr5 > 0.15:
        flags.append(("ÜBERDEHNUNG", -1, "#d29922",
                      f"Div-CAGR 5J {div_cagr5:.1%} > 15% — Ausschüttungswachstum überhitzt"))
        malus -= 1

    # 7 – INTEREST-PRESSURE: Zinsaufwand > 30% des FCF
    if interest_exp > 0 and fcf > 0 and (interest_exp / abs(fcf)) > 0.30:
        flags.append(("INTEREST-PRESSURE", -1, "#d29922",
                      f"Zinsaufwand = {interest_exp / abs(fcf):.0%} des FCF — belastet Ausschüttungsfähigkeit"))
        malus -= 1

    # 8 – PAYOUT-DISKREPANZ: FCF- vs EPS-Payout > 30pp Abweichung
    if fcf_payout is not None and payout_eps is not None:
        _gap = abs(fcf_payout - payout_eps)
        if _gap > 0.30:
            flags.append(("PAYOUT-DISKREPANZ", -0.5, "#d29922",
                          f"FCF-Payout {fcf_payout:.0%} vs EPS-Payout {payout_eps:.0%} "
                          f"· Lücke {_gap:.0%} — Earnings-Qualität prüfen"))
            malus -= 0.5

    # 9 – WIEDERHOLUNGSTÄTER: >= 2 Kürzungen
    if div_cuts >= 2:
        flags.append(("WIEDERHOLUNGSTÄTER", -1, "#da3633",
                      f"{div_cuts} Kürzungen in {div_years} Jahren — Verlässlichkeit fraglich"))
        malus -= 1

    # 10 – REINVEST-KONFLIKT: hohe Payout-Quote bei starkem Umsatzwachstum
    if fcf_payout is not None and fcf_payout > 0.85 and rev_growth > 0.10:
        flags.append(("REINVEST-KONFLIKT", -1, "#d29922",
                      f"FCF-Payout {fcf_payout:.0%} bei Umsatzwachstum {rev_growth:.0%} — Reinvest-Druck"))
        malus -= 1

    # 11 – ZYKLUS-WARNUNG: zyklischer Sektor + hohe Payout-Quote
    _is_cyclical = any(x in sector for x in
                       ["energy", "basic materials", "consumer cyclical", "industrials"])
    if _is_cyclical and fcf_payout is not None and fcf_payout > 0.70:
        flags.append(("ZYKLUS-WARNUNG", -0.5, "#d29922",
                      f"Zyklischer Sektor · FCF-Payout {fcf_payout:.0%} > 70% — Mid-Cycle prüfen"))
        malus -= 0.5

    # 12 – SONDER-FLAG: abnorm hohe Rendite > 12% (Sonder-Div oder Distress)
    if div_yield > 0.12:
        already_trap = any(f[0] == "DIV-TRAP" for f in flags)
        if not already_trap:
            flags.append(("SONDER-FLAG", -1, "#d29922",
                          f"Div-Rendite {div_yield:.1%} > 12% — Sonder-Dividende oder Distress-Signal prüfen"))
            malus -= 1
    ...
    # ── Verdikt ────────────────────────────────────────────────────────────────
    _has_trap  = any(f[0] == "DIV-TRAP"   for f in flags)
    _has_drain = any(f[0] == "CASH-DRAIN" for f in flags)
    _cov_crit  = fcf_coverage is not None and fcf_coverage < 1.2

    if _has_trap or _has_drain or _cov_crit or malus <= -4:
        verdikt = "TRAP"
        v_color = "#da3633"
        v_icon  = "☠️"
    elif malus < -1 or (fcf_coverage is not None and fcf_coverage < 2.0):
        verdikt = "UNTER DRUCK"
        v_color = "#d29922"
        v_icon  = "⚠️"
    else:
    ...
    # ── Shareholder Yield Netto ────────────────────────────────────────────────
    sh_yield_netto = netto_yield + buyback - sbc_i

    return {
        "active":         True,
        "div_yield":      div_yield,
        "netto_yield":    netto_yield,
        "wht":            wht,
        "country":        country,
        "div_rate":       div_rate,
        ...
        "is_neustart":    is_neustart,
        "flags":          flags,
        "malus":          malus,
        "verdikt":        verdikt,
        "v_color":        v_color,
        "v_icon":         v_icon,
        "sh_yield_netto": sh_yield_netto,
        "buyback":        buyback,
        "sbc_i":          sbc_i,
    }
...
def _render_dividenden_check(j: dict, m: dict):
    """JACK 💰 Dividenden-Check — AUTO-TRIGGER wenn Dividende > 0."""
    dc = j.get("dividenden_check", {})
    if not dc.get("active"):
        return

    sym         = "€" if m.get("currency") == "EUR" else "$"
    div_yield   = dc["div_yield"]
    netto       = dc["netto_yield"]
    wht         = dc["wht"]
    country     = dc.get("country") or "N/V"
    div_years   = dc["div_years"]
    div_cuts    = dc["div_cuts"]
    div_cagr5   = dc.get("div_cagr5")
    is_aristo   = dc["is_aristocrat"]
    is_new      = dc["is_neustart"]
    fcf_cov     = dc.get("fcf_coverage")
    fcf_pay     = dc.get("fcf_payout")
    eps_pay     = dc.get("payout_eps")
    flags       = dc.get("flags", [])
    malus       = dc.get("malus", 0)
    verdikt     = dc["verdikt"]
    v_color     = dc["v_color"]
    v_icon      = dc["v_icon"]
    sh_netto    = dc.get("sh_yield_netto", 0)
    buyback_v   = dc.get("buyback", 0)
    sbc_i_v     = dc.get("sbc_i", 0)
    real_fcf_v  = dc.get("real_fcf", 0)
    div_pay_v   = dc.get("div_payments", 0)

    # Expander-Header
    _cov_txt = f"{fcf_cov:.1f}x Coverage" if fcf_cov is not None else "Coverage N/V"
    with st.expander(
        f"💰 DIVIDENDEN-CHECK — {pct(div_yield)} Rendite · {_cov_txt} · {v_icon} {verdikt}",
        expanded=True
    ):
        # ── Kennzahlen-Reihe ──────────────────────────────────────────────────
        _c1, _c2, _c3, _c4, _c5, _c6 = st.columns(6)

        _cov_delta = ("≥ 2x ✅" if (fcf_cov is not None and fcf_cov >= 2.0)
                      else ("< 1.5x ⚠️" if fcf_cov is not None else ""))
        _cov_dc    = "normal" if (fcf_cov is not None and fcf_cov >= 2.0) else "inverse"

        _c1.metric("Div-Rendite (brutto)", pct(div_yield))
        _c2.metric(f"Netto · QSt {wht:.0%}", pct(netto))
        _c3.metric("FCF-Coverage (Real)",
                   f"{fcf_cov:.1f}x" if fcf_cov is not None else "N/V",
                   delta=_cov_delta, delta_color=_cov_dc)
        _c4.metric("FCF-Payout", pct(fcf_pay) if fcf_pay is not None else "N/V")
        _c5.metric("EPS-Payout",  pct(eps_pay)  if eps_pay  is not None else "N/V")
        _c6.metric("Konsistenz",  f"{div_years}J · {div_cuts} Kürz.")

        st.markdown("---")

        # ── Drei Panels ───────────────────────────────────────────────────────
        _ca, _cb, _cc = st.columns([1.4, 1.4, 1.2])

        # Panel A: Div-Qualität
        with _ca:
            st.markdown("**📈 Dividenden-Qualität**")

            def _dq_row(name, val, ok):
                fc = "#3fb950" if ok else ("#d29922" if val in ("N/V", "0") else "#da3633")
                st.markdown(
                    '<div style="display:flex;justify-content:space-between;'
                    'border-bottom:1px solid #21262d;padding:4px 0;">'
                    '<span style="color:#8b949e;font-size:0.82em;">' + name + '</span>'
                    '<span style="color:' + fc + ';font-size:0.82em;font-weight:600;">'
                    + val + '</span></div>',
                    unsafe_allow_html=True)

            _dq_row("Div-Rendite brutto",       pct(div_yield),             div_yield > 0.02)
            _dq_row("Div-Rendite netto",        pct(netto),                 netto > 0.015)
            _dq_row("FCF-Coverage (Real FCF)",
                    f"{fcf_cov:.1f}x" if fcf_cov is not None else "N/V",
                    fcf_cov is not None and fcf_cov >= 2.0)
            _dq_row("FCF-Payout",
                    pct(fcf_pay) if fcf_pay is not None else "N/V",
                    fcf_pay is not None and fcf_pay <= 0.75)
            _dq_row("Div-CAGR 5J",
                    pct(div_cagr5) if div_cagr5 is not None else "N/V",
                    div_cagr5 is not None and 0.03 <= div_cagr5 <= 0.12)
            _dq_row("Zahlungsjahre",            f"{div_years}J",            div_years >= 10)
            _dq_row("Kürzungen",                str(div_cuts),              div_cuts == 0)

            # Quellensteuer-Hinweis
            st.markdown(
                f'<div style="color:#8b949e;font-size:0.73em;margin-top:6px;">'
                f'Quellensteuer: {wht:.0%} · Land: {country}</div>',
                unsafe_allow_html=True)

        # Panel B: Flags
        with _cb:
            st.markdown("**🚩 Nachhaltigkeits-Flags**")
            if not flags:
                st.markdown(
                    '<div style="color:#3fb950;font-size:0.83em;padding:6px 0;">'
                    '✅ Keine kritischen Flags erkannt</div>',
                    unsafe_allow_html=True)
            for _fn, _fm, _fc, _fd in flags:
                st.markdown(
                    f'<div style="background:{_fc}11;border-left:3px solid {_fc};'
                    f'border-radius:0 6px 6px 0;padding:6px 10px;margin:3px 0;">'
                    f'<div style="display:flex;justify-content:space-between;align-items:center;">'
                    f'<span style="color:{_fc};font-weight:700;font-size:0.78em;">{_fn}</span>'
                    f'<span style="color:{_fc};font-size:0.75em;font-weight:700;">{_fm:+.1f}</span>'
                    f'</div>'
                    f'<div style="color:#8b949e;font-size:0.73em;margin-top:2px;">{_fd}</div>'
                    f'</div>',
                    unsafe_allow_html=True)
            if malus < 0:
                st.markdown(
                    f'<div style="color:#8b949e;font-size:0.78em;margin-top:8px;">'
                    f'Gesamt-Malus: <b style="color:#d29922;">{malus:+.1f}</b>'
                    f' (Deckel: −4)</div>',
                    unsafe_allow_html=True)

            # Aristocrat / Neustart Badge
            if is_aristo:
                st.markdown(
                    '<div style="background:#3fb95022;border:1px solid #3fb950;border-radius:6px;'
                    'padding:6px 10px;margin-top:8px;">'
                    '<span style="color:#3fb950;font-weight:700;font-size:0.8em;">'
                    '👑 DIV-ARISTOCRAT</span><br>'
                    f'<span style="color:#8b949e;font-size:0.73em;">'
                    f'{div_years}J ununterbrochen · kein Schnitt</span></div>',
                    unsafe_allow_html=True)
            elif is_new:
                st.markdown(
                    '<div style="background:#79c0ff22;border:1px solid #79c0ff;border-radius:6px;'
                    'padding:6px 10px;margin-top:8px;">'
                    '<span style="color:#79c0ff;font-weight:700;font-size:0.8em;">'
                    'NEUSTART-FLAG</span><br>'
                    f'<span style="color:#8b949e;font-size:0.73em;">'
                    f'Dividende erst seit {div_years}J — Track-Record aufbauen</span></div>',
                    unsafe_allow_html=True)

        # Panel C: Verdikt + Shareholder Yield
        with _cc:
            st.markdown("**⚖️ Verdikt**")
            st.markdown(
                ...
                f'<div style="color:{v_color};font-size:2em;line-height:1.1;">{v_icon}</div>'
                f'<div style="color:{v_color};font-weight:900;font-size:1em;'
                f'letter-spacing:1px;margin-top:4px;">{verdikt}</div>'
                f'</div>',
                unsafe_allow_html=True)

            st.markdown("**💵 Shareholder Yield (Netto)**")
            _sy_color = ("#3fb950" if sh_netto > 0.03
                         else ("#d29922" if sh_netto > 0 else "#da3633"))
            _sy_rows = [
                ("Div-Rendite (netto)", f"+{pct(netto)}",    "#e6edf3"),
                ("+ Buyback (est.)",    f"+{pct(buyback_v)}", "#3fb950"),
                (u"− SBC-Verwässerung", f"-{pct(sbc_i_v)}",  "#da3633"),
            ]
            for _sn, _sv, _sc in _sy_rows:
                st.markdown(
                    f'<div style="display:flex;justify-content:space-between;padding:3px 0;">'
                    f'<span style="color:#8b949e;font-size:0.8em;">{_sn}</span>'
                    f'<span style="color:{_sc};font-size:0.8em;">{_sv}</span></div>',
                    unsafe_allow_html=True)
            st.markdown(
                f'<div style="display:flex;justify-content:space-between;'
                f'border-top:1px solid #30363d;padding:5px 0;margin-top:2px;">'
                f'<span style="color:#e6edf3;font-size:0.82em;font-weight:700;">= Gesamt</span>'
                f'<span style="color:{_sy_color};font-size:0.82em;font-weight:700;">'
                f'{pct(sh_netto)}</span></div>',
                unsafe_allow_html=True)

        st.caption(
            f"[LIVE] Div-Rendite · [S1/S4] FCF-Coverage · Quellensteuer {wht:.0%} "
            f"({country}) — Für persönliche Steueroptimierung Steuerberater konsultieren.")
